...
class ProductsFragment : TransactionsBaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        binding.productsFragmentRecycler.run {
            ...
            adapter = viewModel.dataHandler.value?.getProductList()
                ?.let { BasicAdapter(onProductSelected, it) }
        }
    }

    private val onProductSelected: (id: String) -> Unit = {
        if (activity is ActivityManager) {
            (activity as ActivityManager).goToFragment(
                ProductTransactionsFragment.getInstance(it),
